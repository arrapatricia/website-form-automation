...
test.describe.serial('Paramount Direct - Health Care Insurance Suite', () => {
  ...
  test('Complete Hospital Income Benefit Plan Application Flow', async ({ page }) => {
    ...
    const emailAddress = 'qatest_hip@gmail.com';
    ...
    // 4. STEP 3: Click Hospital Income Benefit Plan link
    const hipPlanLink = page.locator('a[href="/health-insurance/hospital-income-benefit-plan"]').first();
    await expect(hipPlanLink).toBeVisible({ timeout: 15000 });
    await hipPlanLink.click();
    await page.waitForLoadState('domcontentloaded');

    // 5. STEP 4: Click Apply Now button
    const applyNowBtn = page.locator('a[href="/apply/hospital-income-benefit-plan"].orange-btn').first();
    await expect(applyNowBtn).toBeVisible({ timeout: 15000 });
    await applyNowBtn.click();
    await page.waitForLoadState('domcontentloaded');

    // 6. STEP 5: Select Plan & Payment Options (Plan 500 - Family = value 252)
    const planDropdown = page.locator('#application_plan_id');
    await planDropdown.selectOption({ value: '252' });
    ...
    // Fill Birthdate (1990) for standard adult eligibility
    ...
    // ====================================================================
    // PANEL 3: Other Information (Insured Persons - Spouse / Child)
    // ====================================================================
    await page.locator('#collapseOther').waitFor({ state: 'visible', timeout: 10000 });

    // Handle Spouse radio if visible
    const spouseNoRadio = page.locator('#legalSpouseNo');
    if (await spouseNoRadio.isVisible().catch(() => false)) {
      await spouseNoRadio.check({ force: true });
      await spouseNoRadio.dispatchEvent('change');
    }

    // Fill Child details if child section is present (Family Plan)
    const childFname = page.locator('#insured_person_firstname_1');
    if (await childFname.isVisible().catch(() => false)) {
      await childFname.fill(childFirstName);
      await childFname.dispatchEvent('input');

      const childLname = page.locator('#insured_person_lastname_1');
      await childLname.fill(childLastName);
      await childLname.dispatchEvent('input');

      await page.locator('#insured_person_fullname_1').evaluate(
        (el: HTMLInputElement, val) => { el.value = val; }, 
        `${childFirstName} ${childLastName}`
      );

      const childBirthdate = page.locator('#insured_person_birthdate1');
      await childBirthdate.evaluate((el: HTMLInputElement) => {
        el.value = '10/10/2018';
      });
      await childBirthdate.dispatchEvent('input');
      await childBirthdate.dispatchEvent('change');
      await childBirthdate.dispatchEvent('blur');
    }
    ...
    const screenshotPath = path.join(SCREENSHOTS_DIR, `hip_submitted_${Date.now()}.png`);
    await page.screenshot({ path: screenshotPath, fullPage: true });

    process.stdout.write('\n=========================================\n');
    process.stdout.write(`✅ HOSPITAL INCOME BENEFIT PLAN APPLICATION SUBMITTED!\n`);
