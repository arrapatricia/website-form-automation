...
test.describe.serial('Paramount Direct - Health Care Insurance Suite', () => {
  ...
  test('Complete PrimeCARE Cash Plan Application Flow', async ({ page }) => {
    ...
    const emailAddress = 'qatest_pcp@gmail.com';
    ...
    // 4. STEP 3: Click PrimeCARE Cash Plan link
    const pcpCashPlanLink = page.locator('a[href="/health-insurance/primecare-cash-plan"]').first();
    await expect(pcpCashPlanLink).toBeVisible({ timeout: 15000 });
    await pcpCashPlanLink.click();
    await page.waitForLoadState('domcontentloaded');

    // 5. STEP 4: Click Apply Now button
    const applyNowBtn = page.locator('a[href="/apply/primecare-cash-plan"].orange-btn').first();
    ...
    await planDropdown.selectOption({ value: '1' }); // Plan 500
    ...
    // Fill Birthdate (1965) to satisfy PrimeCARE Cash Plan age restrictions (50-75 yo)
    ...
    // Click 'same_with_insured' and explicitly update hidden input value
    ...
    const screenshotPath = path.join(SCREENSHOTS_DIR, `pcp_submitted_${Date.now()}.png`);
    await page.screenshot({ path: screenshotPath, fullPage: true });

    process.stdout.write('\n=========================================\n');
    process.stdout.write(`✅ PRIMECARE CASH PLAN APPLICATION SUBMITTED!\n`);
